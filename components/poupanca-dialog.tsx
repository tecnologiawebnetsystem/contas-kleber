...
interface PoupancaDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onUpdate: () => void
}

export function PoupancaDialog({ open, onOpenChange, onUpdate }: PoupancaDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex items-center justify-between rounded-lg bg-amber-500/5 border border-amber-500/20 px-3 py-2">
          ...
          <span className="text-sm font-bold text-amber-500">{formatarMoeda(total)}</span>
        </div>
